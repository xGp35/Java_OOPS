...
public class GameHelper {
    ...
    public ArrayList<String> placeStartup(int startupSize){
        ...
        while (!success & attempts++ < MAX_ATTEMPTS) {      // main search loop
            ...
            if (startupFits(startupCoords, increment)) {        // startup fits in the grid ?
                success = coordsAvailable(startupCoords);       // ... and locations aren't taken?
            } // end for
        } // end while
        savePositionToGrid(startupCoords);                      // cords passed checks, save
    ...
    private boolean coordsAvailable(int[] startupCoords) {
        for (int coord : startupCoords) {           // check all potential positons
    ...
    private ArrayList<String> convertCoordsToAlphaFormat(int[] startupCoords) {
        ...
        for (int index : startupCoords) {                        // for each grid coordinate
            String alphaCoords = getAlphaCoordsFromIndex(index); // turn it into "a0" style
            alphaCells.add(alphaCoords); // add to a list
        }
        return alphaCells; // return the "a0"-style cords
